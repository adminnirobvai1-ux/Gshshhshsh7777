...
def auto_close_browser(session_key):
    ...
    if sess:
        try:
            print(f"[*] ২৪ ঘন্টা পার হওয়ায় সেশন {session_key} বন্ধ করা হচ্ছে...")
            sess["driver"].quit()
        except Exception:
            pass
...
# আপনার দেওয়া উইনজি-মার্টিনগেল অটোমেশন ইঞ্জিন
...
let tInp = document.querySelector('input[placeholder*="TARGET PROFIT"]');
let sInp = document.querySelector('input[placeholder*="TOTAL STEPS"]');
let gBtn = Array.from(document.querySelectorAll('button')).find(b => (b.innerText || '').toUpperCase().includes('START'));
...
# ==========================================
# ৫. লগইন ও স্বয়ংক্রিয় গেম রিডাইরেকশন
# ==========================================
def process_login(chat_id, session_key, phone, password, status_msg_id):
    ...
    if not fill_ok:
        stop_anim.set()
        bot.edit_message_text("❌ লগইন ফিল্ড পাওয়া যায়নি অথবা পেজ লোড হতে দেরি হয়েছে।", chat_id=chat_id, message_id=status_msg_id)
        return
    ...
    if login_status == "SUCCESS" or login_status == "PENDING":
        ...
        try:
            ...
            print(f"[!] গেম পেজ লোডে ত্রুটি: {e}")

        # পেজ ও ব্যালেন্স লোড হওয়ার জন্য অপেক্ষা
        time.sleep(6)
        live_bal = None
        for _ in range(15):
            try:
        ...
        bal_str = f"{live_bal:.2f} ৳" if (live_bal is not None) else "সনাক্ত করা যায়নি"
...
# ==========================================
# ৬. টেলিগ্রাম মেনু ও কলব্যাক হ্যান্ডলার
# ==========================================
def show_profile_menu(chat_id, message_id=None):
    ...
    text = "⚙️ **প্রোফাইল মোড নির্বাচন করুন:**\n\n" \
           ...
           "• **Skip Profile:** সম্পূর্ণ ফ্রেশ আলাদা প্রোফাইল।"
...
def send_welcome(message):
    ...
    bot.send_message(
        message.chat.id, 
        "🚀 **লগইন ও অটোমেশন কন্ট্রোল প্যানেল**\n\nঅনুগ্রহ করে প্ল্যাটফর্ম নির্বাচন করুন:", 
        reply_markup=markup,
        parse_mode="Markdown"
    )
...
def handle_callbacks(call):
    ...
    if data in ["site_amarclub", "site_dkwin"]:
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            "✍️ ফিক্সড প্রোফাইলের জন্য একটি নাম লিখে পাঠান (যেমন: RDP1):",
            chat_id=chat_id,
            message_id=call.message.message_id,
            parse_mode="Markdown"
        )
        ...
        txt = "🌐 **বর্তমানে সচল থাকা ব্রাউজারসমূহ:**" if found else "বর্তমানে কোনো সচল ব্রাউজার নেই।"
        ...
        markup.add(InlineKeyboardButton("🔙 ফিরে যান", callback_data="btn_back_to_prof"))
        bot.edit_message_text("🗑️ ডিলিট করার জন্য প্রোফাইল নির্বাচন করুন:", chat_id=chat_id, message_id=call.message.message_id, reply_markup=markup)
...
# ==========================================
# ৭. টেক্সট মেসেজ ইনপুট ও অটোমেশন ট্রিগার
# ==========================================
...
def handle_text_inputs(message):
    ...
    if step == "WAITING_NEW_PROFILE_NAME":
        ...
        bot.send_message(
            chat_id,
            f"📱 নাম্বার: `{text}` সংরক্ষিত।\n\n🔑 এবার আপনার **পাসওয়ার্ড (P)** লিখে পাঠান:",
            parse_mode="Markdown"
        )
    ...
    # ধাপ ২: টার্গেট অ্যামাউন্ট গ্রহণ
    ...
    # ধাপ ৩: স্টেপ গ্রহণ ও জাভাস্ক্রিপ্ট অটোমেশন স্টার্ট
        ...
        try:
            ...
            if started:
                bot.send_message(
                    ...
                    f"⚡ ব্রাউজারের ব্যাকগ্রাউন্ডে অটোমেটিক ট্রেডিং চলছে। টার্গেট পূর্ণ হলে স্বয়ংক্রিয়ভাবে অ্যালার্ট আসবে।",
                    parse_mode="Markdown"
                )
            else:
                bot.send_message(chat_id, "⚠️ অটোমেশন প্যানেল তৈরি হয়েছে, তবে স্টার্ট হতে বিলম্ব হচ্ছে। VNC স্ক্রিনে ব্রাউজার চেক করতে পারেন।")
        except Exception as e:
